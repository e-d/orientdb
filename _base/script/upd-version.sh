...
TFILE="/tmp/out.tmp.$$"
for filename in $(/bin/grep -r "$2" --include "*.java" --include "*.htm*" --include "*.xml" --exclude-dir "apidocs" --exclude-dir "target" --exclude-dir "test-output" $1|/bin/cut -f 1 -d :)
do
  if [ -f $filename -a -r $filename ]; then
    #/bin/cp -f $filename ${filename}.old
    /bin/sed "s/$2/$3/g" "$filename" > $TFILE && mv $TFILE "$filename"
  else
